...
export const AuthPage: React.FC<AuthPageProps> = ({ onNavigate }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        toast.error('Invalid credentials. Try: WikiEditor2024, SourceVerifier, or AdminUser');
      }
    } catch (error) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const success = await register(registerUsername, registerEmail, registerCountry);
      if (success) {
        toast.success('Registration successful! Welcome to WikiSourceVerifier.');
        onNavigate('landing');
      } else {
