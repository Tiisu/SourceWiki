...
export const adminApi = {
  ...
  getAnalytics: (params?: {
    ...
    return api.get(`/admin/analytics?${query.toString()}`);
  },
  ...
  getSubmissions: (params?: {
    ...
    return api.get(`/admin/submissions?${query.toString()}`);
  },

  overrideSubmission: (id: string, data: {
    status: string;
    adminNotes?: string;
    reason?: string;
  }) => api.put(`/admin/submissions/${id}/override`, data),

  deleteSubmission: (id: string, reason?: string) => 
    api.delete(`/admin/submissions/${id}`),

  // Export Functionality
  exportSubmissions: async (filters: {
    format?: 'csv' | 'json';
    status?: string;
    category?: string;
    country?: string;
    ...
    const url = `${API_URL}/admin/export/submissions?${query.toString()}`;
    ...
    try {
      ...
      let filename = `submissions_export_${new Date().toISOString().split('T')[0]}`;
  ...
  getExportFilters: () => api.get('/admin/export/filters'),

  // Batch Operations
  batchApproveSubmissions: (data: {
    submissionIds: string[];
    verifierNotes?: string;
  }) => api.post('/admin/batch/approve', data),

  batchRejectSubmissions: (data: {
    submissionIds: string[];
    verifierNotes?: string;
  }) => api.post('/admin/batch/reject', data),

  batchDeleteSubmissions: (data: {
    submissionIds: string[];
  }) => api.post('/admin/batch/delete', data),

  batchUpdateStatus: (data: {
    submissionIds: string[];
    status: 'approved' | 'rejected';
    verifierNotes?: string;
  }) => api.post('/admin/batch/update-status', data),

  getBatchOperationPreview: (data: {
    submissionIds: string[];
  }) => api.post('/admin/batch/preview', data),
};
